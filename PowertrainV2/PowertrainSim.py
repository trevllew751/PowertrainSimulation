...
parse = pd.read_excel("Powertrain Simulation Data.xlsx", engine="openpyxl",
                      sheet_name=["Motors", "MotorControllers", "Batteries"])
# Breaks the parse into individual sheets. The sheet types are dataframes
motors = parse["Motors"].values
motor_controllers = parse["MotorControllers"].values
batteries = parse["Batteries"].values


def all_batt_configs(controller, v_min, i_min, v_max, i_max):
    ...
    for battery in batteries:
        ...
        for v in np.arange(v_min, v_max, 1):
            for i in np.arange(i_min, i_max, 1):
                ...
                # TODO: Fix parallel cells calculations
                ...
                packs.append(pack)
        with open(f"./packs/{controller}.csv", 'w', newline='') as packs_csv:
...
def motor_controller_configs():
    for mc in motor_controllers:
        #                mc model    v_min  i_min  v_max  i_max
        all_batt_configs(str(mc[0]), mc[1], mc[3], mc[2], mc[4])


def main():
    # all_batt_configs(100, 100, 105, 110)  # Dummy test
    motor_controller_configs()
